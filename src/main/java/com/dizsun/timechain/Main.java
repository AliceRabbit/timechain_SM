...
import com.dizsun.timechain.service.CloseHook;
import com.dizsun.timechain.service.HTTPService;
import com.dizsun.timechain.service.NTPServer;
	...
	public static void main(String[] args) {
		...
		try {
			...
			new Timer().schedule(new TimerTask() {
				@Override
				public void run() {
					if (config.getLocalHost().equals(config.getMainNode())) {
						return;
					}
					p2pService.connect(config.getMainNode());
					logger.info("连接至：" + config.getMainNode());
				}
			}, 2000);
