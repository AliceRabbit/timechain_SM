...
public class P2PService implements ISubscriber {
	...
	public void initP2PServer(int port) {
		...
		this.sm2Util = SM2Util.getInstance();
		logger.info("initing rsaUtil -> pubprikey............");
		sm2Util.init(config.getLocalHost() + "." + config.getP2pPort());
	...
	private boolean checkACK(ACK ack) {
		...
		if (!sm2Util.getPubPriKey().verify(ack.getPublicKey(), ack.getPublicKey() + ack.getVN(), ack.getSign())) {
			return false;
		}
		return true;
	}
