...
//            context.addServlet(new ServletHolder(new MineBlockServlet()), "/mineBlock");
			...
			context.addServlet(new ServletHolder(new PeersNumberServlet()), "/peers_number");
			context.addServlet(new ServletHolder(new NTPCostTimeServlet()), "/NTPcost_time");
	...
	//
	private class NTPCostTimeServlet extends HttpServlet {
		@Override
	...
	//
	private class PeersNumberServlet extends HttpServlet {
		@Override
		protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
			resp.setCharacterEncoding("UTF-8");
			resp.getWriter().println(JSON.toJSONString(dateUtil.getTimeFromRC()));
