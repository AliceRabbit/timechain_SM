...
/**
 * 本类整合了对本地区块链的基本数据操作 其中getLatestBlock()、addBlock()、replaceChain()、rollback()
...
//        }
	...
	/**
	 * 计算区块hash 将(索引+前一个区块hash+时间戳+数据)进行hash
	 *
	 * @param index
	 * @param previousHash
	 * @param timestamp
	...
	private String calculateHash(int index, String previousHash, long timestamp, String data) {
		...
		return CryptoUtil.getSM3(builder.toString());
	}
	...
	public void generateNextBlock(String blockData, String localHost) {
		...
		// int
		// proof=createProofOfWork(previousBlock.getProof(),previousBlock.getHash());
		Block newBlock = new Block(nextIndex, previousBlock.getHash(), nextTimestamp, blockData, nextHash,
				R.getViewNumber(), localHost);
...
//            sqlUtil.addBlock(newBlock);
			...
			R.endConsensus();
			// P2PService.java 177
			// MessageHelper.java 118
			// above two unnecessary?
...
//            sqlUtil.replaceChain(newBlocks);
			...
			// 删除当前存储的区块链文件
//			persistenceService.blockchainPersistenceClear(config.getLocalHost() + "." + config.getP2pPort());
	 ...
	 * 持久化并裁剪区块链
	 */
	public void blockchainPersistenceAndCut() {
		persistenceService.blockchainPersistenceAppend(config.getLocalHost() + "." + config.getP2pPort(),
				blockChain.subList(0, blockChain.size() - 1));
