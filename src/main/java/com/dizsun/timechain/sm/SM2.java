...
public class SM2 
{
	...
	public byte[] sm2GetZ(byte[] userId, ECPoint userKey)
	{
		...
		byte[] p = com.dizsun.timechain.sm.Util.byteConvert32Bytes(ecc_a);
		sm3.update(p, 0, p.length);
		
		p = com.dizsun.timechain.sm.Util.byteConvert32Bytes(ecc_b);
		sm3.update(p, 0, p.length);
		
		p = com.dizsun.timechain.sm.Util.byteConvert32Bytes(ecc_gx);
		sm3.update(p, 0, p.length);
		
		p = com.dizsun.timechain.sm.Util.byteConvert32Bytes(ecc_gy);
		sm3.update(p, 0, p.length);
		
		p = com.dizsun.timechain.sm.Util.byteConvert32Bytes(userKey.getX().toBigInteger());
		sm3.update(p, 0, p.length);
		
		p = com.dizsun.timechain.sm.Util.byteConvert32Bytes(userKey.getY().toBigInteger());
		sm3.update(p, 0, p.length);
	...
	public void sm2Sign(byte[] md, BigInteger userD, ECPoint userKey, com.dizsun.timechain.sm.SM2Result sm2Result) 
	{
...
//		        kp = this.ecc_point_g.multiply(k);
	...
	public void sm2Verify(byte md[], ECPoint userKey, BigInteger r, BigInteger s, com.dizsun.timechain.sm.SM2Result sm2Result)
    {
        sm2Result.R = null;
        BigInteger e = new BigInteger(1, md);
